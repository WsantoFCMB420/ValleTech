...
@section('topbar-title', 'STAFF // EDIT TECHNICIAN')

@section('content')
    <div class="vt-page-header d-flex align-items-center justify-content-between">
        <div>
            <h1>Editar Técnico</h1>
            <p>Modificando: <span style="color:var(--teal)">{{ $tecnico->nombre }} {{ $tecnico->apellido }}</span></p>
        </div>
                ...
                <div class="vt-card-body">
                    <form action="{{ route('tecnicos.update', $tecnico) }}" method="POST">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="vt-form-group"><label class="vt-label">Nombre *</label><input type="text"
                                        name="nombre" class="vt-input" value="{{ old('nombre', $tecnico->nombre) }}"
                                        required></div>
                            </div>
                            <div class="col-md-6">
                                <div class="vt-form-group"><label class="vt-label">Apellido *</label><input type="text"
                                        name="apellido" class="vt-input" value="{{ old('apellido', $tecnico->apellido) }}"
                                        required></div>
                            </div>
                            <div class="col-12">
                                <div class="vt-form-group"><label class="vt-label">Email *</label><input type="email"
                                        name="email" class="vt-input" value="{{ old('email', $tecnico->email) }}"
                                        required></div>
                            </div>
                            <div class="col-md-6">
                                <div class="vt-form-group"><label class="vt-label">Teléfono</label><input type="text"
                                        name="telefono" class="vt-input" value="{{ old('telefono', $tecnico->telefono) }}">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="vt-form-group"><label class="vt-label">Especialidad *</label>
                                    <select name="especialidad" class="vt-input vt-select" required>
                                        @foreach (['Hardware', 'Software', 'Redes', 'Eléctrico', 'Mecánico', 'Hidráulico', 'General'] as $esp)
                                            <option value="{{ $esp }}"
                                                {{ old('especialidad', $tecnico->especialidad) == $esp ? 'selected' : '' }}>
                                                {{ $esp }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="vt-form-group"><label class="vt-label">Estado *</label>
                                    <select name="estado" class="vt-input vt-select" required>
                                        @foreach (['Activo', 'Inactivo'] as $est)
                                            <option value="{{ $est }}"
                                                {{ old('estado', $tecnico->estado) == $est ? 'selected' : '' }}>
                                                {{ $est }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div style="display:flex;gap:12px;margin-top:8px">
                            <button type="submit" class="btn-vt"><i class="bi bi-floppy"></i> Actualizar Técnico</button>
